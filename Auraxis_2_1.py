# =========================
# Auraxis 2.1 — Final Version
# Multi-Par + Microtrajectória + Sweep/IPI Detalhado
# =========================
...
st.set_page_config(page_title="Auraxis 2.1 Final", layout="wide")
st.title("Auraxis 2.1 — Simulator Fusion Layer (Final)")
...
def compute_confidence(candle):
    body, total_range = abs(candle['Close'] - candle['Open']), candle['High'] - candle['Low']
    if total_range == 0: return 0.3
...
def simulate_sweep_IPI(df):
    # Sweep/IPI simplificado: variação rápida do candle e pressão institucional aproximada
    ipi = []
...
# -------------------------
tabs = st.tabs(pares_disponiveis)

for i, par in enumerate(pares_disponiveis):
    with tabs[i]:
        st.subheader(f"Par: {par}")
        ...
        fig.update_layout(title=f"Auraxis 2.1 — {par} ({periodo})", xaxis_title="Tempo", yaxis_title="Preço",
